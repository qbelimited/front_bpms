...
function ActivatePlate({id, handleClose, open}) {
    const [loading, setLoading] = useState(false)

    const handleActivate = () =>{
        setLoading(true)
        postService.activatePlate(id).then(
            (response) => {
            ...
            <div className=' mt-4'>
                <p className=' text-center font-bold mb-3'>Click OK to confirm Deactivation</p>
                    <div className=' flex justify-center'>
                        <div className=' flex justify-between'>
                            <div>
                                <button onClick={handleClose} className='  bg-red-800 p-3 rounded-md m-2 text-white'>
                                    No
                                </button>
                                <button onClick={handleActivate} className='bg-green-800  p-3 m-2 rounded-md text-white'>
                                  {loading? 'Loading' :  'Yes'}
                                </button>
                            </div>
                        </div>
